...
## Как работает:
**1. Получаете токен аккаунта в MAX (необязательно, есть авторизация по номеру телефона)**  
...
Вместо токена можно указать только `MAX_PHONE` — при первом запуске сервис запросит код из SMS, а если на аккаунте включена двухфакторная аутентификация, то ещё и пароль  
Полученный токен и идентификатор устройства сохраняются в файл `max_session.json`, поэтому при следующих запусках код запрашиваться не будет. Если токен отзовут, сервис сам запросит новый код (при указанном `MAX_PHONE`)
...
- [ ] Определение ФИ автора сообщения и указание его в пересланном сообщении
